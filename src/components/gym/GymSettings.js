...
export default function GymSettings() {
  const [gympassword, setPassword] = useState('');
  ...
  const handleChangePassword = async (e) => {
    ...
    if (!gympassword || !newPassword || !confirmPassword) {
    ...
    try {
      const response = await fetch('http://localhost:5000/api/gym/updategympassword', {
        method: 'PUT',
        headers: {
          'Content-Type': 'application/json',
          "auth-token": localStorage.getItem('token')
        },
        body: JSON.stringify({ gympassword, newPassword })
      });
